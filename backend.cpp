...
/*another implementation of event-filter*/
MyXcbEventFilter::MyXcbEventFilter(int a, void* p) : be_p(static_cast<BackEnd*>(p)) {
...
bool MyXcbEventFilter::nativeEventFilter(const QByteArray& eventType, void* message, long* result) {
    ...
    if (kev != nullptr) {
        ...
        DLOG(INFO) << "KeyCode:" << keycode << ";keystate:" << key_modifier.toStdString();
        ...
        */
        KeySym keysym = XkbKeycodeToKeysym(xdisplay, keycode, 0, keystate & ShiftMask ? 1 : 0);
        ...
        if (idx_comma > 0) {
            DLOG(INFO) << "combine" << combine_keystring.left(idx_comma).toStdString() << idx_comma;
        } else {
            int n = combine_keystring.indexOf('+');
            int len = combine_keystring.length();
            DLOG(INFO) << "no-modifier" << combine_keystring.right(len - n - 1).toStdString() << len << n;
        }
...
bool ShortcutListener::eventFilter(QObject* obj, QEvent* ev) {
    if (QEvent::MouseMove == ev->type()) {
        qDebug() << "event filter::get mouse move";
        return true;
    }
    return false;
}
...
void ShortcutListener::listenTo(QObject* object) {
    if (object) {
        qDebug() << "listenTo called with obj";
